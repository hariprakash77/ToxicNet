...
def adminlogin():
    ...
    if request.method == 'POST':
       if request.form['uname'] == 'admin' or request.form['password'] == 'admin':
           ...
           conn = mysql.connector.connect(user='root', password='', host='localhost', database='1socialnetpyemo')
           # cursor = conn.cursor()
           cur = conn.cursor()
...
def newuser():
    ...
    if request.method == 'POST':
        ...
        mydb = mysql.connector.connect(user='root', password='', host='localhost', database='1socialnetpyemo')
        cursor = mydb.cursor()
...
def Friend1():
    ...
    for x in data:
        uid = x[0]
        print(uid)
    mydb = mysql.connector.connect(host="localhost", user="root", password="", database="1socialnetpyemo")
    mycursor = mydb.cursor()
    ...
    if data12 is None:
        ...
        aList = []
        mydb1 = mysql.connector.connect(host="localhost", user="root", password="", database="1socialnetpyemo")
...
def post1():
    if request.method == 'POST':
        ...
        conn = mysql.connector.connect(user='root', password='', host='localhost', database='1socialnetpyemo')
        mycursor = conn.cursor()
        mycursor.execute("insert into sharetb values('"+ str(fnew) +"','" + uname + "','" + place + "','" + f.filename + "','"+uname+"')")
        conn.commit()
        conn.close()
        ...
        for x1 in data:
            ...
            mycursor = conn.cursor()
            mycursor.execute(
                "insert into sharetb values('"+ str(fnew) +"','" + uname + "','" + place + "','" + f.filename + "','" + fname + "')")
            conn.commit()
            conn.close()
...
def cmt():
    ...
    return render_template('Comment.html', data=data, data1=data1,smile1=smile1,smile2=smile2,smile3=smile3, smile4=smile4, smile5=smile5 ,smile6=smile6)
...
def postcomment():
    if request.method == 'POST':
        ...
        if( j >= 1):
            ...
            conn = mysql.connector.connect(user='root', password='', host='localhost', database='1socialnetpyemo')
            mycursor = conn.cursor()
            mycursor.execute("insert into comtb values('','" + uname + "','" + frname + "','" + comm + "','1','"+id+"','" + str(em1) + "','" + str(em2) + "','" + str(em3) + "','" + str(em4) + "','" + str(em5) + "','" + str(em6) + "')")
            conn.commit()
            conn.close()
        else:
            conn = mysql.connector.connect(user='root', password='', host='localhost', database='1socialnetpyemo')
            mycursor = conn.cursor()
            mycursor.execute(
                "insert into comtb values('','" + uname + "','" + frname + "','" + comm + "','0','" + id + "','" + str(em1) + "','" + str(em2) + "','" + str(em3) + "','" + str(em4) + "','" + str(em5) + "','" + str(em6) + "')")
            conn.commit()
            conn.close()

        conn = mysql.connector.connect(user='root', password='', host='localhost', database='1socialnetpyemo')
        cursor = conn.cursor()
        cursor.execute("SELECT sum(Ccount) as count  FROM comtb WHERE uname  ='" + uname + "' and frname='"+ str(session['uname']) +"'  ")
        data2 = cursor.fetchone()
        if data2:
            ccount = data2[0]
        ...
        if(ccount ==2):
            ...
            mycursor = mydb.cursor()
            mycursor.execute(
                "update frlist set status='Blocked' where uname='" + str(uname) + "' and frname='" + str(session['uname']) + "'")
            mydb.commit()
            mydb.close()
            ...
            mycursor1 = mydb1.cursor()
            mycursor1.execute(
                "update frlist set status='Blocked' where uname='" + str(session['uname']) + "' and frname='" + str(uname) + "'")
            mydb1.commit()
            mydb1.close()
